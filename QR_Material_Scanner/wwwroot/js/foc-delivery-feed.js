...
function openModalFetchDelivery() {
    ...
    dataArray = [];
    var URL = "/api/P20551/C_Get_FOC_Delivery_Details/" + txtDeliveryChallanNoFetch.value.trim();
    ...
    fetch(URL)
        ...
        .then(function (data) {
            ...
            clearAllData('foc-delivery-material-details');
            if (data.length <= 0) {
            ...
            for (var key in data) {
                ...
                //writeData('foc-delivery-material-details', data[key]);
                //DeliveryFetchRecord = true;

                if (data[key].Quantity_To_Be_Picked <= 0) {
                    GRAlreadyDone = true;
                    SubmitButtonScan.style.display = 'none';
                    $('#actionSheetForm').modal('hide');
                    $("#DialogAlert").modal();
...
function readDeliveryAllData() {
    readAllData('foc-delivery-material-details')
        .then(function (data) {
            console.log(data);
...
function readDeliveryScanAllData() {
    readIndexData('scan-foc-delivery-material', 'Document_No', txtDeliveryChallanNo.value)
        .then(function (data) {
            console.log(data);
            //Scantable(data);
...
$('#btnDeleteYesNo').click(function () {
    ...
    deleteItemFromData('scan-foc-delivery-material', id);
            ...
            .then(function (data) {
                ...
                if (e.data.length >= 10) {
                    ...
                    if (txtDeliveryChallanNo.value.trim() !== "") {
                        ...
                        $("body").append(
                            "<div id='foc-delivery-toast' class='toast-box toast-top bg-warning tap-to-close'>"
                            +
                        ...
                        toastbox('foc-delivery-toast', 3000);
                    }
                    //readDeliveryScanAllData();
                ...
                if (deliveryfind.length > 0) {
                    for (var i = 0; i < deliveryfind.length; i++) {
                        ...
                        if (deliveryScanCount < deliveryfind[i].Quantity_To_Be_Picked) {
                            ...
                            writeData('scan-foc-delivery-material', DeliveryScanObj);
                        }
                        else {
                            $("body").append(
...
SubmitButton.addEventListener('click', function (event) {
    ...
    readIndexData('scan-foc-delivery-material', 'Document_No', txtDeliveryChallanNo.value)
        .then(function (data) {
            ...
            //            BulkwriteData('sync-foc-delivery-aws', data)
            //                .then(function () {
            //                    return sw.sync.register('sync-foc-delivery-aws');
            //                })
            //                .then(function () {
            //                    //alert("Sync Started");
        ...
        navigator.serviceWorker.ready
            .then(function (sw) {
                ...
                BulkwriteData('sync-foc-delivery-aws', data)
                    ...
                    .then(function () {
                        $("body").append(
                            "<div id='foc-delivery-success-toast' class='toast-box toast-top bg-success  tap-to-close'>"
                            +
                            "<div class='in'><ion-icon name='checkmark-circle' class='text-light md hydrated' role='img' aria-label='success'></ion-icon><div class='text'>"
                            +
                            "Added Successfull"
                        ...
                        toastbox('foc-delivery-success-toast', 4000);
                    })
                    .catch(function (err) {
                        console.log(err);
                        alert("Backround Sync failed" + err);
