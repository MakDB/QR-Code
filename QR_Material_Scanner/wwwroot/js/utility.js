...
//------------------------- Delivery------------------------------------------
    if (!db.objectStoreNames.contains('delivery-material-details')) {
        ...
        //------------------------- FOC Delivery------------------------------------------
        if (!db.objectStoreNames.contains('foc-delivery-material-details')) {
            //db.createObjectStore('posts', { keyPath: 'GR_Number'});
            var store = db.createObjectStore('foc-delivery-material-details', { keyPath: 'id', autoIncrement: true });
            store.createIndex('Document_No', 'Document_No', { unique: false });
        }

        if (!db.objectStoreNames.contains('scan-foc-delivery-material')) {
            //db.createObjectStore('posts', { keyPath: 'GR_Number'});
            var store = db.createObjectStore('scan-foc-delivery-material', { keyPath: 'Serial_Number' });
            store.createIndex('Material_Number', 'Material_Number', { unique: false });
            store.createIndex('Document_No', 'Document_No', { unique: false });
            //store.createIndex('MaterLine_No', ['Material_Number', 'Line_Item_Number'], { unique: true });
        }

        if (!db.objectStoreNames.contains('sync-foc-delivery-aws')) {
            db.createObjectStore('sync-foc-delivery-aws', { keyPath: 'Serial_Number' });
        }
...
const SortScanObj = (sortscanobj) => {
    ...
    let obj = {};
    obj = {
        Serial_Number: sortscanobj.Serial_Number.trim(),
