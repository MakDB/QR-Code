...
//workbox.precaching.precacheAndRoute(self.__precacheManifest || [
    ...
    { url: '/foc_delivery', revision: version },
                    ...
                    .then(function (res) {
                        return caches.open('dynamic')
                            .then(function (cache) {
                                ...
                                if ((args.url.pathname != '/login') && (args.url.pathname != '/Login') && (args.url.pathname != '/Logout') && (args.url.pathname != '/ForgotPassword') && (args.url.pathname != '/ChangePassword') && (args.url.pathname != '/ResetPassword') && (!args.event.request.url.indexOf('/storelocator/')) && (args.url.pathname.toLowerCase() != '/storelocator') && (args.url.pathname != '/StoreCodeSearch') && (args.url.pathname.toLowerCase() != '/storecodesearch'))
                                {
...
self.addEventListener('sync', function (event) {
    ...
    if (event.tag === 'sync-foc-delivery-aws') {
        console.log('[Service Worker] Syncing new Posts - Delivery');
        event.waitUntil(
            readAllData('sync-foc-delivery-aws')
                .then(function (data) {
                    for (var dt of data) {
                        fetch('/api/P20551/C_Insert_Material_Transaction', {
                            method: 'POST',
                            ...
                            .then(function (res) {
                                console.log('Sent data', res);
                                if (res.Status_Code === "200") {
                                    deleteItemFromData('sync-foc-delivery-aws', res.Serial_Number); // Isn't working correctly!
                                    deleteItemFromData('scan-foc-delivery-material', res.Serial_Number); //
